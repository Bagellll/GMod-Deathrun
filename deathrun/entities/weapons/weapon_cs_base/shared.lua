...
if SERVER then
	...
	function SWEP:Think()
		...
		self:GetOwner().LastSpeed = self:GetOwner().LastSpeed or 0

		local speed = self:GetOwner():GetVelocity():Length()

		local accel = math.abs(speed - self:GetOwner().LastSpeed)
		self:GetOwner().LastSpeed = speed
		self:GetOwner().CurAccel = accel
		...
		if GetConVar("dr_unlimited_ammo"):GetBool() then
			local remaining = self:GetOwner():GetAmmoCount( self:GetPrimaryAmmoType() ) or self:Ammo1() or 0
			if remaining < self.Primary.ClipSize then
				self:GetOwner():GiveAmmo( self.Primary.ClipSize, self:GetPrimaryAmmoType(), true )
			end
		end
...
function SWEP:PrimaryAttack()
	...
	self.Reloading = false
	self:SetNWBool( "ReloadingShotgun", false )

	timer.Simple( self.Primary.Delay*0.8, function() 
		if self and not self.Reloading then
	...
	if self.LaserBeams then
		self:EmitSound( "weapons/airboat/airboat_gun_energy1.wav", 75, 100 - 50*(math.min(self.Primary.Damage*self.Primary.NumShots,100)/100), 1 )
	else
...
function SWEP:CSShootBullet( dmg, recoil, numbul, cone )
	...
	--local accfrac = (1 + self:GetOwner().CurAccel/5) -- double the cone when walking at full pace

	accfrac = ( self:GetOwner():GetVelocity():Length()/self:GetOwner():GetWalkSpeed() )*0.075
	...
	--if SERVER then
		local tr = util.TraceLine({
			...
			filter = {self, self:GetOwner(), self},
		})
