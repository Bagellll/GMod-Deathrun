...
function SWEP:Think()
        ...
        if self.Primed == 1 and not self:GetOwner():KeyDown(IN_ATTACK) and self.PrimaryThrow then
                if self.Throw < CurTime() then
						...
						if(SERVER) then
							self:GetOwner():EmitSound("radio/ct_fireinhole.wav")
						end
        ...
        elseif self.Primed == 1 and not self:GetOwner():KeyDown(IN_ATTACK2) and not self.PrimaryThrow then
                if self.Throw < CurTime() then
                        self.Primed = 2
                        self.Throw = CurTime() + 1.5
...
function SWEP:ThrowFar()
        ...
        local ent = ents.Create (self.NadeClass)
                        ...
                        local v = self:GetOwner():GetShootPos()
                                v = v + self:GetOwner():GetForward() * 2
                        ent:SetPos( v )

        --ent:SetAngles ((Vector(math.random(1,100),math.random(1,100),math.random(1,100))):Angle())
        ent:SetOwner( self:GetOwner() )
        ent.GrenadeOwner = self:GetOwner()
        ent:Spawn()

        local phys = ent:GetPhysicsObject()

        phys:ApplyForceCenter( self:GetOwner():GetVelocity() )
        phys:ApplyForceCenter(self:GetOwner():GetAimVector() *750 + Vector(0,0,70) )
        phys:AddAngleVelocity(Vector(math.random(-200,200),math.random(-200,200),math.random(-200,200)))

        self:GetOwner():StripWeapon(self:GetOwner():GetActiveWeapon():GetClass())
...
function SWEP:ThrowShort()
        ...
        phys:ApplyForceCenter( self:GetOwner():GetVelocity() )
        phys:ApplyForceCenter(self:GetOwner():GetAimVector() *50 )
        phys:AddAngleVelocity(Vector(math.random(-200,200),math.random(-200,200),math.random(-200,200)))
