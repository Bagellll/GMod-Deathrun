...
function SWEP:PrimaryAttack()
	...
	if tr.HitPos:Distance( self:GetOwner():GetShootPos() ) < 75 then
		if tr.Entity then
			if SERVER then
				local keyvalues = tr.Entity:GetKeyValues()
				if keyvalues.classname == "func_breakable" then
					--tr.Entity:Input("RemoveHealth", self:GetOwner(), self, self.Primary.Damage)
				end
			end
		end
	else
		if SERVER then
			self:GetOwner():EmitSound(self.MissSound, 100, 100, 1)
		end
	end
...
function SWEP:Deploy()
	...
	self:GetOwner():EmitSound( Sound("weapons/knife/knife_deploy1.wav") )
